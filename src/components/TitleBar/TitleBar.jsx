...
export default class TitleBar extends Component {
  ...
  toggleMenuExpansion() {
    ...
    document.getElementById('expanding-menu').classList.toggle('title-bar__controls--closed');
    this.setState({
  ...
  filterSubmit() {
    ...
    let { minWeight, maxWeight } = this.state;
    minWeight = Number.parseInt(minWeight.split(' ')[0].replace(',', ''), 10);
    maxWeight = Number.parseInt(maxWeight.split(' ')[0].replace(',', ''), 10);
    onFilter(startDate, endDate, minWeight, maxWeight);
  }
  ...
  render() {
    ...
    const selectionMenus = new Array(4).fill(null);
    const {
      openSelection,
      minWeight,
      maxWeight,
    ...
    if (openSelection === 0) {
      selectionMenus[0] = (
        <div className="title-bar__selection title-bar__open-selection">
          <Calendar
            value={date}
            onChange={value => this.chooseStartDate(moment(value).format('DD/MM/YYYY'))}
          />
        </div>
      );
    } else if (openSelection === 1) {
      selectionMenus[1] = (
        <div className="title-bar__selection title-bar__open-selection">
          <Calendar
            value={date}
            onChange={value => this.chooseEndDate(moment(value).format('DD/MM/YYYY'))}
          />
        </div>
      );
    } else if (openSelection === 2) {
      selectionMenus[2] = (
        <div className="title-bar__selection title-bar__open-selection">
          <DropDownBox
            items={weightSelectionItems}
            onSelect={item => this.chooseMinWeight(item)}
          />
        </div>
      );
    } else if (openSelection === 3) {
      selectionMenus[3] = (
        <div className="title-bar__selection title-bar__open-selection">
          <DropDownBox
            items={weightSelectionItems}
            onSelect={item => this.chooseMaxWeight(item)}
          />
        </div>
      );
    }
        ...
        <div className="title-bar">
          <div className="title-bar__content">
            <div className="title-bar__title-bar">
              ...
              <div className="title-bar__button-container">
                <button type="button" onClick={() => this.toggleMenuExpansion()}><FontAwesomeIcon icon={faEllipsisH} /></button>
              </div>
            </div>
            <form className="title-bar__form">
              <div id="expanding-menu" className="title-bar__controls title-bar__controls--closed">
